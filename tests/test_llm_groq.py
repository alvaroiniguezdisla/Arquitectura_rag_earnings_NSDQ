...
Qué se valida:
    ...
    - Timeout: una petición que excede HTTP_TIMEOUT devuelve error controlado.

Estrategia:
    Se inyecta un mock de ToolManager directamente al constructor de GroqLLM
    (inyección de dependencias) y se mockea requests.Session.post para simular
    respuestas de la API de Groq. No se realizan llamadas HTTP reales.
"""
import json
import pytest
...
class TestGroqLLM:
    ...
    def _make_llm(self, mock_tool_manager=None):
        """Helper: crea un GroqLLM con API key fake y ToolManager mockeado."""
        from src.rag.generation.llm_groq import GroqLLM
        tm = mock_tool_manager or MagicMock()
        return GroqLLM(api_key="fake-key-for-testing", tool_manager=tm)

    @patch("requests.Session.post")
    def test_chat_direct_response(self, mock_post, mock_groq_response):
    ...
    @patch("requests.Session.post")
    def test_chat_with_tool_call(self, mock_post,
        ...
        mock_tm = MagicMock()
        mock_tm.execute_tool_call.return_value = json.dumps([
            {"text": "Apple revenue was $91.8B", "score": 0.95}
        ])

        llm = self._make_llm(mock_tool_manager=mock_tm)
        result = llm.chat_with_tools("¿Ingresos de Apple en Q1 2020?")

        assert mock_post.call_count == 2
        mock_tm.execute_tool_call.assert_called_once()
        assert isinstance(result, str)
    ...
    def test_timeout_returns_error_message(self, mock_post):
        """Un timeout HTTP devuelve error controlado, no se queda colgado."""
        import requests as real_requests
        mock_post.side_effect = real_requests.exceptions.ReadTimeout(
            "Read timed out. (read timeout=30)"
        )

        llm = self._make_llm()
        result = llm.chat_with_tools("¿Revenue de Apple?")
    ...
    def test_missing_api_key_raises(self, mock_getenv):
        ...
        with pytest.raises(ValueError, match="GROQ_API_KEY"):
            GroqLLM(api_key=None, tool_manager=MagicMock())
